 ...
 *      Author: kokke/tiny-AES-c, Roman Savrulin and DF4IAH
 */
...
const uint8_t const_Rb[16]    = { 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x87 };
const uint8_t const_Zero[16]  = { 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00 };
...
static void cryptoAesCmac_leftshift_onebit(const uint8_t* input, uint8_t* output) {
  uint8_t overflow = 0;

  for (int8_t i = cryptoAesCmac_LAST_INDEX; i >= 0; i--) {
    output[i]  = input[i] << 1;
    output[i] |= overflow;
...
static void cryptoAesCmac_generate_subkey(const uint8_t* key, uint8_t* K1, uint8_t* K2) {
  uint8_t L[cryptoAesCmac_BLOCK_SIZE];
  ...
  if ((L[0] & 0x80) == 0) {       /* If MSB(L) = 0, then K1 = L << 1 */
    ...
    cryptoAesCmac_leftshift_onebit(L, tmp);
    cryptoAesCmac_xor_128(tmp, const_Rb, K1);
  }

  if ((K1[0] & 0x80) == 0) {
    cryptoAesCmac_leftshift_onebit(K1, K2);

  } else {
    cryptoAesCmac_leftshift_onebit(K1, tmp);
    cryptoAesCmac_xor_128(tmp, const_Rb, K2);
  }
}
...
void cryptoAesCmac(const uint8_t aesKey128[16], volatile uint8_t* input, uint32_t length, uint8_t* outMac) {
  ...
  cryptoAesCmac_generate_subkey(aesKey128, K1, K2);

  uint32_t n = (length + cryptoAesCmac_LAST_INDEX) / cryptoAesCmac_BLOCK_SIZE; /* n is number of rounds */
  ...
  if (n == 0) {
    ...
    if ((length % cryptoAesCmac_BLOCK_SIZE) == 0) { /* last block is a complete block */
      flag = 1;
  ...
  if (flag) { /* last block is complete block */
    cryptoAesCmac_xor_128(&input[cryptoAesCmac_BLOCK_SIZE * (n - 1)], K1, M_last);

  } else {
    cryptoAesCmac_padding(&input[cryptoAesCmac_BLOCK_SIZE * (n - 1)], padded, length % cryptoAesCmac_BLOCK_SIZE);
    cryptoAesCmac_xor_128(padded, K2, M_last);
  }
